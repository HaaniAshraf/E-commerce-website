...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('/popularProduct')
      ...
      .catch(error => console.error('Error fetching most popular product:', error));
  });
...
document.addEventListener('DOMContentLoaded', function () {
  fetch('/recentUpdates')
      ...
      .then(data => {
          ...
          recentUpdatesList.appendChild(createListItem(`${data.latestSignup.name} signed up at ${new Date(data.latestSignup.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}`));
          recentUpdatesList.appendChild(createListItem(`${data.pendingOrders} orders pending to be delivered.`));
          recentUpdatesList.appendChild(createListItem(`New product ${data.latestProduct.name} added to ${data.latestProduct.category}`));
