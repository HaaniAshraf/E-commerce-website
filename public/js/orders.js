    document.addEventListener("DOMContentLoaded", function () {
        const cancelOrderButton = document.querySelectorAll(".cancelOrder");
                ...
                if (reason) {
                    try {
                        ...
                        if (data.success) {
                            ...
                            const orderStatusElements = document.querySelectorAll(`.order-item-right[data-order-id="${orderId}"] .orderStatus span`);
                            orderStatusElements.forEach((statusElement) => {
                                statusElement.style.color = 'rgb(249, 57, 57)';
                                statusElement.innerText = 'Cancelled';
                                button.style.display = 'none';

                                const deliveryDateElement = document.querySelector(`.order-item-right[data-order-id="${orderId}"] .deliveryDate`);
