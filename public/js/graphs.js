...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("/sales")
    ...
    .then((data) => {
      ...
      // Initializes an array allMonthsData with 12 objects, each representing a month and initialized with zero total sales.
      const allMonthsData = Array.from({ length: 12 }, (_, index) => ({
        month: monthNames[index],
        totalSales: 0,
      }));
      // Iterates over the sorted data array and updates the corresponding month's total sales in the allMonthsData array.
      data.forEach((entry) => {
      ...
      // Creates arrays months and salesData by extracting data starting from the first month with non-zero sales.
      const months = allMonthsData.slice(startIndex).map((entry) => entry.month);
      const salesData = data.map((entry) => entry.totalSales);
      // Inserts "Jan" at the beginning of the months array and 0 at the beginning of the salesData array
      months.unshift("Jan");
      salesData.unshift(0);
      ...
      const updatedChart = new ApexCharts(
  ...
  fetch("/userSignups")
    ...
    .then((data) => {
      ...
      const signupsData = Array.from({ length: 12 }, (_, index) => {
          // Find the corresponding entry for the current month
        const monthData = monthlySignups.find(
          (entry) => entry._id === index + 1
        );
          // If data is found, assign total signups; otherwise, assign 0
